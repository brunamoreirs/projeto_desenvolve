...
import { enhanceText as aiEnhanceText, isAvailable } from "../services/geminiService";
...
export function useAIEnhancement() {
  ...
  const enhanceText = useCallback(
    async (
      ...
      setLoadingState({ isLoading: true, loadingText: "Processando com IA..." });
      ...
      try {
        ...
        if (response.enhancedText && response.enhancedText !== text) {
          ...
          toast.success(
            `Texto melhorado! ${response.improvements.length} melhorias aplicadas`
          );
        } else {
        ...
        toast.error(
          "Erro na IA - Não foi possível processar o texto. Tente novamente."
        );
      } finally {
  ...
  const isAIAvailable = useCallback(() => isAvailable(), []);

  return { enhanceText, loadingState, isAIAvailable };
}
